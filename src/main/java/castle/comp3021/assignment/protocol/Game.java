...
public abstract class Game implements Cloneable {
    ...
    public abstract void showHistoryMove();
    ...
    public @Nullable Piece getPiece(@NotNull Place place) {
        if (place.x() < 0 || place.x() >= this.configuration.getSize()) {
            return null;
        }
        if (place.y() < 0 || place.y() >= this.configuration.getSize()) {
            return null;
        }
    ...
    public Game clone() throws CloneNotSupportedException {
        ...
        for (var player :
                cloned.getConfiguration().getPlayers()) {
            playerCloneMap.put(player, player.clone());
        }
        cloned.configuration = this.configuration.clone();
        cloned.board = new Piece[this.configuration.getSize()][];
        for (int i = 0; i < this.configuration.getSize(); i++) {
            ...
            for (int j = 0; j < this.configuration.getSize(); j++) {
                ...
                if (cloned.board[i][j] != null)
                    cloned.board[i][j].player = playerCloneMap.get(this.board[i][j].player);
            }
        }
        cloned.currentPlayer = currentPlayer == null ? null : playerCloneMap.get(currentPlayer);
        return cloned;
    }
}